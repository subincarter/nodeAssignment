...
exports.getFeeds = async (req, res, next) => {
    ...
    if (req.user.role == 'superadmin') {
        ...
        feeds = await Feeds.findAll({ where: { admin_user_id: req.user.id } });
    } else {
        if (!req.user.access.includes('read')) {
            res.send({ status: false, message: 'You dont have access to read the feed' });
        }
    ...
    if (feeds) {
        res.send({ status: true, feeds: feeds });
    }
...
exports.feedAllocation = async (req, res, next) => {
    ...
    if (req.user.role == 'superadmin') {
        ...
        res.send({ status: false, message: `Access denied! user dont have to allocate` });
    }
    if (userRole == 'admin') {
