...
exports.feedAllocationToAdmin = async (req, res, next) => {
    ...
    if (req.user.role === 'superadmin') {
        canUpdate = true;
    } else if (req.user.role === 'admin' && req.user.access.includes("update")) {
        const getFeedById = await Feeds.findOne({ where: { id: id } })
        if (getFeedById.admin_user_id != req.user.userID) {
            res.send({ status: false, message: `Cannot update feed  =${id} to user. Access denied!` });
        }
        canUpdate = canUpdateUser = true;
    }
    if (canUpdate) {
        await Feeds.update({ admin_user_id: adminUserID }, { where: { id: feedID } }).then((data) => { })
    }else{
        await Feeds.update({ admin_user_id: adminUserID }, { where: { id: feedID } }).then((data) => { })
    }
    res.send(req.body.);
