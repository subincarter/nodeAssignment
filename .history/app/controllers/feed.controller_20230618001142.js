...
exports.feedAllocation =  (req, res, next) => {
    ...
    if (req.user.role == 'superadmin') {
        ...
        const getFeedById =  Feeds.findOne({ where: { id: id } })
        if (getFeedById.admin_user_id != req.user.userID) {
            res.send({ status: false, message: `Cannot update feed  =${id} to user. Access denied!` });
        }
    }
    if (userRole == 'admin') {
         Feeds.update({ admin_user_id: userID }, { where: { id: feedID } })
        .then((data) => { })
    } else {
        console.log(feedID);
         Feeds.update({ user_id: userID }, { where: { id: feedID } })
        .catch(err =>{res.send('feed not found')})
        console.log('csa')
    }
}