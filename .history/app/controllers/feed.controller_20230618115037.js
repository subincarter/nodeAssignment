...
exports.getFeeds = async (req, res, next) => {
    ...
    if (req.user.role == 'superadmin') {
        ...
        if (!req.user.access.includes('read')) {
            res.send({ status: false, message: 'You dont have access to read the feed' });
        }
...
exports.feedAllocation = async (req, res, next) => {
    ...
    if (req.user.role == 'superadmin') {
        ...
        const getFeedById = await Feeds.findOne({ where: { id: id } })
        if (getFeedById.admin_user_id != req.user.userID) {
            return res.send({ status: false, message: `Cannot update feed  =${id} to user. Access denied!` });
        }
    } else if (req.user.role === 'user') {
        res.send({ status: false, message: `Access denied! user dont have access to allocate feeds` });
    }
    if (userRole == 'admin') {
        await Feeds.update({ admin_user_id: userID }, { where: { id: feedID } })
            .then((num) => {
                if (num == 1) {
                    res.send({ status: true, message: 'allocated' });
                }
                res.send({ status: false, message: 'feed not found' });
            })
            .catch(err => {
                console.log(err);
            })
