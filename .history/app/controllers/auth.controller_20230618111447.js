...
exports.updateAccess = async (req, res, next) => {
    ...
    if(req.user.role == 'user'){
        res.status(501).send({ status: false, message: 'access denied' });
    }
    await Users.update({ access: access }, { where: { id: userdID } }).catch(err => {
        console.log(err)
        res.send({ status: false, message: 'failed to update the user access' });
    });
    res.send({ status: true, message: 'user access updated' })
}